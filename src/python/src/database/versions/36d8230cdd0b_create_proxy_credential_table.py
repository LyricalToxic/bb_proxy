...
def upgrade():
    ...
    op.execute(
        "CREATE TRIGGER IF NOT EXISTS UpdateOnCurrentTimestamp1 AFTER UPDATE ON proxy_credentials "
        "BEGIN "
        "UPDATE proxy_credentials set updated_at=CURRENT_TIMESTAMP WHERE id=id; "
        "END;"
    )
