...
const {
    PYTHON_INTERPRETER, // SCRAPY_SCRIPT,
    PROJECT_PREFIX, MAX_MEMORY_RESTART, PYTHON_CWD, PM2_LOG_DIRECTORY, NODEJS_SCRIPT,
...
const commands = [{
    name: `${PROJECT_PREFIX}_proxy`,
    script: "bbc_server.py",
    args: "",
    interpreter: PYTHON_INTERPRETER,
    instances: 1,
    autorestart: true, // cron_restart: "0 * * * *",
},];
...
Array.from([producers, consumers, commands, spiders]).map(t => {
    t.reduce((a, v) => {
        ...
        }, v, (v.hasOwnProperty('cron_restart')) ? {
            cron_restart: v.cron_restart, autorestart: false,
        } : null,));
        return a
    }, apps)
});
